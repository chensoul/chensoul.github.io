...
date: 2026-04-03 10:00:00+08:00
draft: true
...
Agoda 通过两步日志架构、集群拆分、监控审计系统和动态负载均衡等策略，成功解决了 Kafka 处理每天 1.8 万亿条事件时面临的扩展挑战。

### 要点回顾

- **两步日志架构**：将运维关注点从开发团队分离，客户端库写磁盘 + 独立 Forwarder 进程转发到 Kafka，以 10 秒延迟换取更好的弹性和灵活性。
- **按用例拆分集群**：多个小型专用集群替代单一大型集群，实现故障隔离和差异化配置（如保留周期、吞吐量）。
- **监控与审计**：JMXTrans + Graphite + Grafana 用于监控，自定义审计系统确保数据完整性，审计数据存入独立集群。
- **Lag-aware 负载均衡**：生产者根据分区 lag 动态调整消息分发；消费者在高 lag 时主动退出触发重新平衡，解决硬件异构和工作负载不均问题。

### 边界与易误读

- **Lag-aware Producer 适用场景**：主要适用于无 key 消息；对于有 key 消息，这种策略可能导致乱序处理风险。
- **Lag-aware Consumer 的触发条件**：不是所有 lag 增加都需要消费者退出，只有在消费者因硬件性能不足导致持续 lag 时才有效。如果 lag 是因为消费者数量不足，退出反而加剧问题。
- **过度配置问题普遍存在**：在异构硬件或不均匀工作负载场景下，轮询策略都会导致资源浪费，这不仅限于 Kafka。

### 术语对照

- **Partitioner**：消息生产者端决定消息分配到哪个分区的策略
- **Assignor**：消费者组内决定分区如何分配给消费者的策略
- **Lag**：消费者落后于生产者消息进度的度量
- **SASL**：Simple Authentication and Security Layer，Kafka 支持的认证机制
- **JMX**：Java Management Extensions，Java 平台的管理和监控接口
