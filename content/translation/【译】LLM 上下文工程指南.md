...
### 一句话核心
LLM 性能随输入长度增加而下降（context rot），上下文工程通过 Write、Select、Compress、Isolate 四大策略管理上下文窗口，确保模型获得恰好所需的信息。

### 要点回顾
- **Context Rot**：LLM 性能在输入超过某个长度后急剧下降，从 95% 准确率暴跌至 60%
- **Lost in the Middle**：LLM 对输入开头和结尾关注最多，中间关注度显著下降，准确率可下降 30%+
- **四大策略**：
  - Write：外部存储（scratchpads、长期记忆）
  - Select：RAG 检索最相关内容
  - Compress：对话总结、修剪、工具输出压缩
  - Isolate：多代理系统，每个代理专注上下文

### 边界与易误读
- 百万 token 上下文窗口是营销，有效上下文长度通常小得多
- 更多上下文≠更好，不相关信息会成为干扰项
- 压缩会永久丢失信息，需权衡

### 术语与延伸
- **Context Rot**：LLM 性能随输入长度增加而退化的现象
- **RAG**：Retrieval-Augmented Generation，检索增强生成
- **Scratchpads**：代理在长期任务中保存中间计划的外部存储
- **CLAUDE.md**：Claude Code 的持久指令记忆文件
