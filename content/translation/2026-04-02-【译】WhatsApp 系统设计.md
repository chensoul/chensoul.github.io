...
这篇文章涵盖了 WhatsApp 实时消息系统的核心设计：

**核心组件**：
| 组件 | 职责 |
|------|------|
| 聊天服务器 | WebSocket 连接、消息路由 |
| 消息队列 | 异步处理、解耦 |
| Redis | 用户连接缓存、存在跟踪 |
| Cassandra | 消息存储（高写入吞吐） |
...
| 粘性会话 | 避免服务器跳跃 |
| Redis 缓存 | 微秒级路由决策 |
| NoSQL 消息存储 | 高写入吞吐、易扩展 |
| 心跳机制 | 检测僵尸连接 |

**消息传递流程**：
1. 客户端 → 聊天服务器（WebSocket）
2. 服务器 → 消息队列（异步）
3. 消息存储 → 数据库
4. 服务器 → 接收者（WebSocket 或推送通知）

**关键洞察**：
- WebSocket 是实时消息的最佳协议
- 服务发现对动态扩展至关重要
- 离线消息需要 30 天保留
- 媒体直接上传到 S3（绕过聊天服务器）
- 心跳防止僵尸连接积累

理解这些设计对构建大规模实时消息系统至关重要。
