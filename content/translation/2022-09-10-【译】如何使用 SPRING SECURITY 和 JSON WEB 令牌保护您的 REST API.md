...
![Application Architecture: 401 Unauthorized](spring-security-jwt/01.webp)
...
![Application Architecture: JSON Web Token (JWT)](spring-security-jwt/02.webp)

然后，客户端将存储 JWT，并且每个后续请求将通过 Authorization 标头传递它。当服务器应用程序收到带有 JWT 的请求时，它将验证它是否是有效令牌，如果是，则允许请求继续。

![Application Architecture: Request with JSON Web Token (JWT)](spring-security-jwt/03.webp)
...
![Spring Initializer](spring-security-jwt/04.webp)
...
![Spring Security Login](spring-security-jwt/05.webp)
...
![Spring Security HTTP Basic](spring-security-jwt/06.webp)
...
![Application Architecture: JSON Web Token (JWT)](spring-security-jwt/07.webp)
...
![Httpie with Authorization](spring-security-jwt/10.webp)
