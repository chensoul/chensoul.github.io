...
date: 2026-03-27 15:38:00+08:00
draft: true
...
这篇文章的重点，不是讨论某个特定 agent 框架，而是把生产环境中常见的 agentic 失败，归纳成 6 类系统设计错误。全文一直在强调：问题往往不在模型，而在工程决策本身。

原文的结构也很清晰：前五个错误分别对应上下文、架构复杂度、自主性选择、输出处理和规划机制，第六个错误则把视角拉回到评测体系，说明为什么这些问题如果没有持续衡量，就会在生产中不断累积。

其中一个很重要的阅读线索是，作者几乎每一节都遵循同一套节奏：先指出常见做法，再说明为什么它在生产环境里会失败，最后给出更保守、工程化的修正方式。也因此，这篇文章更像一份工程诊断清单，而不是“如何一键构建 agent”的操作指南。

原文后面还有课程推广、评论与分享引导等内容，按 skill 已略。正文真正保留的，是对 6 类失败模式本身的论述与图示。