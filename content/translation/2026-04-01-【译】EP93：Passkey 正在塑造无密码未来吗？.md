...
tags: [ "passkey", "authentication", "security" ]
...
- 私钥存储在设备上
- 无需密码，使用生物识别解锁
- 跨平台支持（Windows、macOS、iOS、ChromeOS）

**缓存系统 4 种问题**：
| 问题 | 原因 | 解决方案 |
|------|------|----------|
| 雷群问题 | 大量键同时过期 | 随机过期时间、限制核心数据 |
| 缓存穿透 | 键不存在 | 缓存空值、布隆过滤器 |
| 缓存击穿 | 热键过期 | 不设置过期时间 |
| 缓存崩溃 | 缓存宕机 | 断路器、缓存集群 |

**字节序**：
- **小端**：最低有效字节在前（Intel x86）
- **大端**：最高有效字节在前（网络通信、文件存储）
