...
date: 2019-12-28 00:00:00+08:00
draft: true
slug: jms-message-selector-filtering-messages
categories: ["translation"]
tags: ["jms"]
description: "在本文中，你将了解如何在 JMS 消息消费者中使用消息选择器来过滤消息。"
canonicalURL: "https://jstobigdata.com/jms/jms-message-selector-filtering-messages/"
---

发送给接收方的消息通常需要根据某些标准进行过滤。JMS 提供消息选择器，允许 JMS 消费者根据消息头指定其感兴趣的消息。在本文中，你将学习使用 JMS 消息选择器来过滤消息。

这是一篇高级教程，在继续阅读本文之前，请确保你对 JMS 有基本的了解。如果你是 JMS 新手，请查看 [JMS 点对点消息传递模型文章](https://jstobigdata.com/jms/jms-point-to-point-messaging-in-action/)。
...
你已经了解了基于 JMS 属性的消息过滤。现在，请看下面的示例以了解基于 Header 的消息过滤。`ERROR` 级别日志或优先级在 5 到 9 之间的消息将传递给消费者。
...
- 数值按原样显示，带有可选的 `+` 或 `-` 符号。例如，`range BETWEEN -200 AND +30`。你也可以直接提及 `30` 而不是 `+30`。
- 数字也可以用科学计数法表示，例如 `-7E3`、`6.2`。
...
- 标识符区分大小写。
- 任何以 `JMSX` 开头的名称都是 JMS 定义的属性。
- 任何以 `JMS_` 开头的名称都是提供商特定的属性。
- 任何不包含 `JMS` 的名称（标识符）都是应用程序定义的属性。
...
你了解了 JMS 消息选择器及其用于过滤传递给客户端的消息的方式。它在实际应用中非常有用，特别是从性能角度来看。你的应用程序可以将大量处理负载委托给 JMS 提供程序，并确保只向其传递必要的消息。

> 本文为学习目的的个人翻译，译文仅供参考。
>
> 原文链接：[JMS Message Selectors in Action to Filter Messages](https://jstobigdata.com/jms/jms-message-selector-filtering-messages/)。
>
