...
这篇文章涵盖了 OOP、多线程、编译、API 测试和 API 设计的核心概念：

**OOP 四大支柱**：
| 支柱 | 说明 | 示例 |
|------|------|------|
| 抽象 | 隐藏实现细节 | Vehicle 类的抽象方法 |
| 封装 | 包装数据和访问限制 | 私有字段 + 公共 getter/setter |
| 继承 | 子类继承父类 | Car 继承 Vehicle |
| 多态 | 方法根据对象表现不同 | 互换使用 |
...
| 类型 | 编译 | 执行 | 示例 |
|------|------|------|------|
| 编译语言 | 机器码 | CPU 直接执行 | C、C++、Go |
| 字节码语言 | 字节码 | JVM 执行 | Java、C# |
| 解释语言 | 不编译 | 解释器运行时 | Python、JS |

**9 种 API 测试**：
冒烟、功能、集成、回归、负载、压力、安全、UI、模糊测试
