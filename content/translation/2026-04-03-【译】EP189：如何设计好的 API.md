...
这篇文章涵盖了 API 设计和 AWS 学习：

**好 API 原则**：
| 原则 | 说明 |
|------|------|
| 幂等性 | GET/PUT/DELETE 幂等 |
| 版本控制 | /v1/、/v2/ |
| 命名 | 名词不是动词 |
| 安全 | HTTPS、JWT |
| 分页 | limit、offset |
...
**大数据管道**：
- Ingestion → Data Lake → Computation → Warehouse → Presentation

**AWS 学习路径**：
- 基础 → 核心服务 → 数据库 → 安全 → DevOps → 认证
...
**RAG 应用**：
- Ingestion：S3 → Lambda → 嵌入 → 向量数据库
- Querying：问题→嵌入→检索→LLM→答案
