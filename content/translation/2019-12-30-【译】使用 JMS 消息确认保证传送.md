...
tags: ["jms"]
description: "在本文中，你将了解 JMS 消息确认模式及其如何保证消息传送。"
canonicalURL: "https://jstobigdata.com/jms/guaranteed-delivery-using-jms-message-acknowledgement/"
---

JMS 通过 JMS 消息确认（确认模式）确保消息传递的可靠性。如果会话已进行事务处理，则该 `commit()` 方法会自动处理消息确认。否则，该方法将处理 `rollback()`。在本文中，我们将假设会话未进行事务处理。这是一个高级主题，请确保你对 JMS 有基本的了解。如果你是 JMS 新手，可以参考使用 JMS [发送和接收消息](https://jstobigdata.com/jms/send-and-receive-message-in-jms/)。
...
这是创建 `Session` 或 `JMSContext` 时设置的默认确认模式。你可以手动指定 `AUTO_ACKNOWLEDGE` 模式，如下所示：
...
当你将会话模式设置为 `CLIENT_ACKNOWLEDGE`，客户端会通过调用其确认方法来确认消息。确认已使用的消息会自动确认已收到其会话已传送的所有消息。
...
从上面的输出可以看出，对于消息生产者来说，不需要确认消息。但作为一种良好做法，你应该始终确认（第 19 行）。

另一方面，`messageConsumer1` 接收消息但不确认，因此不会从 JMS 服务器中删除该消息。因此，`messageConsumer3` 能够接收相同的消息，但这次它确认了该消息，因此会从 JMS 服务器中删除该消息。因此，`messageConsumer2` 不会收到该消息，并且会在发生超时时抛出 `NullPointerException`。
...
实际上，你很可能会坚持使用 **AUTO_ACKNOWLEDGE** 模式。但是，深入了解其他模式也总是好的。这都是 JMS 消息确认的一部分，使用它来确保有保证的消息传递。
