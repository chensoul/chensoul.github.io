---
title: "【译】JMS 消息模型"
date: 2019-11-10 17:32:00+08:00
draft: false
slug: jms-message-model
categories: ["translation"]
tags: ["jms"]
description: "消息是 JMS 的核心，无论是事件还是业务数据。消息由标头、自定义属性和正文组成。"
canonicalURL: "https://jstobigdata.com/jms/jms-message-model/"
---

消息是 JMS 的核心，无论是事件还是业务数据。消息由标头、自定义属性和正文（payload）组成。这是你将在 JMS 消息模型中学习的内容。

![JMS Message Model解剖](01.webp)

JMS 消息由 Header、Properties 和 Body（Payload）组成。下面详细讨论它们：

- **消息头**是关于消息的元数据。消息头有两种类型：由开发人员设置的头和由 JMS 提供者设置的头。
- **消息属性**是用于向消息添加可选标头字段的附加标头字段。属性字段主要有三种类型：
  - 应用程序特定属性 - 允许设置应用程序特定的自定义标头。
  - 标准属性 - JMS 定义的标准属性。
  - JMS 提供者特定属性。
- **消息体**是发送到目的地的有效负载。JMS 支持多种类型（格式）的有效负载。
...
更多讨论请参阅**[JMS 消息优先级教程](https://jstobigdata.com/jms/jms-prioritize-messages/)**。
...
- 属性值可以是 `boolean`、`byte`、`short`、`int`、`long`、`float`、`double`、`String`。
...
在本教程中，我通过示例简要讨论了 JMS 消息模型。请在下面的评论中告诉我你的反馈或问题。我将在单独的文章中讨论**[消息优先级](https://jstobigdata.com/jms/jms-prioritize-messages/)**和**消息请求 - 响应**。
