...
date: 2026-04-03 10:30:00+08:00
draft: true
...
本文详细介绍了 Figma 数据库架构演进的完整历程，从单机 RDS 到水平分片的完整技术路径。

**核心要点：**

1. **渐进式演进**：Figma 没有选择推倒重来，而是在现有 PostgreSQL 基础上逐步演进，先垂直分区争取时间，再水平分片实现长期扩展
2. **垂直分区先行**：先按表的重要性和访问量进行垂直拆分，为后续工作争取了一年时间窗口
3. **逻辑分片与物理分片分离**：这是 Figma 方案的精髓，允许先验证应用层逻辑（可简单回滚），再执行物理数据迁移（高风险）
4. **自研 DBProxy**：通过自定义查询引擎实现分片路由，支持单分片查询高效执行，对 scatter-gather 查询进行限流保护

**技术亮点：**

- **Colo 概念**：将相关表分组，共享分片键和布局，支持跨表连接和事务
- **Shadow Application 框架**：预测生产流量在不同分片键下的表现，指导查询语言设计
- **优化逻辑复制**：通过移除目标库索引再重建的方式，将复制时间从数周缩短到数小时

**适用场景：**

如果你面临类似的数据库 scaling 问题，Figma 的经验表明：不要急于引入复杂的分布式数据库，先充分利用现有架构的扩展能力（垂直分区、读写分离），同时为未来的水平分片做好准备（应用层抽象、查询规范）。

**值得注意的细节：**

- Figma 选择自建而非采购，主要基于团队熟悉度、现有投资和控制力考虑
- 他们优先支持 90% 的常见查询模式，避免在查询引擎中处理最复杂的边缘情况
- 通过 PgBouncer 层先行分区，为客户端路由错误提供缓冲，降低迁移风险
