---
title: "【译】Docker 速成课程"
date: 2026-03-31 16:08:00+08:00
draft: true
...
这篇文章把 Docker 的来龙去脉讲得比较清楚，有几个关键点：

- **Docker 的核心创新不在容器技术本身**（Linux 容器早就有了），而在于**容器镜像的打包和分发格式**
- **Docker vs 虚拟机**：容器共享主机内核，通过 namespaces 和 cgroups 实现隔离，而不是虚拟化硬件
- **Docker 为什么赢了**：Cloud Foundry 等 PaaS 厂商没意识到容器可以作为开发者工具外部化，Docker 抓住了这个机会
- **后续发展**：Docker 后来想扩展到完整 PaaS（Swarm、Compose），但 Google 等巨头入局后，Kubernetes 最终成为编排标准

对于现在的开发者来说，Docker 已经是基础设施的一部分，但理解它的历史和设计理念，有助于更好地理解云原生生态的演进。
