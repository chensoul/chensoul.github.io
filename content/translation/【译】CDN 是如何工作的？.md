---
title: "【译】CDN 是如何工作的？"
date: 2026-03-31 16:26:00+08:00
draft: true
...
这篇文章把 CDN 的工作流程讲得比较清楚，核心就几点：

- **CDN 的本质**：地理上分布的边缘服务器网络，让用户从最近的节点获取内容
- **DNS 是关键**：通过 CNAME 记录把域名指向 CDN，再由 CDN 负载均衡器选择最优边缘节点
- **分层缓存**：边缘→区域→中央→源站，逐级回源
- **内容类型**：静态内容（页面、图片、视频）+ 动态内容（边缘计算结果）

实际使用中：
- 静态资源（CSS、JS、图片、视频）几乎都该走 CDN
- 动态内容（个性化页面、实时数据）也可以通过边缘计算优化
- CDN 选型要看节点分布、缓存策略、价格等因素

理解 CDN 的工作原理，对优化网站性能和设计全球部署架构很有帮助。
