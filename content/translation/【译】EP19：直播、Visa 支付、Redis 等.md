...
**直播技术**：
- 核心流程：捕获→压缩编码→分段→自适应比特率→CDN 分发→解码播放
- 关键协议：RTMP（老牌）、HLS（Apple 专用）、DASH（开放标准）
- 优化方向：边缘计算、存储选型（对象存储适合视频持久化）

**支付系统**：
- 4 方模型（Visa/Mastercard）：发卡行、收单行、卡网络、商家分离
- 3 方模型（AMEX）：闭环系统，效率更高但扩展慢
- 盈利模式：交换费是发卡行主要收入，卡网络收评估费和使用费

**思考题**：
- 卡网络对大商家和小商家收取相同的交换费吗？（答案：通常不同，大商家有议价能力）

这些是支付和流媒体系统设计的基础知识，理解它们对设计相关系统至关重要。
