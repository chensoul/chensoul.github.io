...
这篇文章介绍了 Uber 的 CacheFront 集成缓存系统：
...
**关键技术**：
1. **缓存模式**：查询引擎集成 Redis
2. **缓存失效**：基于 Flux CDC（binlog）
3. **一致性验证**：Compare cache 系统（99.99% 一致）
4. **跨区域预热**：复制键而非值
5. **多 Redis 集群**：避免单点故障
6. **滑动窗口断路器**：短路故障节点

**成果**：
- P75 延迟 ↓75%，P99.9 延迟 ↓67%
- 缓存一致性 99.9%
- 每秒 4000 万次请求
- 成本从 6 万 CPU 核心降至 3000 Redis 核心

CacheFront 展示了如何在大规模下构建高效、可扩展的集成缓存系统。
