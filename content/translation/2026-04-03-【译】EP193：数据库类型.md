...
tags: [ "database", "sql", "nosql", "vector" ]
...
| 类型 | 用途 | 示例 |
|------|------|------|
| 关系型 | 结构化数据 | MySQL、PostgreSQL |
| 列式 | 分析 | ClickHouse、Redshift |
| 键值 | 快速查找 | Redis、DynamoDB |
| 内存 | 缓存 | Redis、Memcached |
| 宽列 | 大数据 | Cassandra、HBase |
| 时间序列 | 指标、日志 | InfluxDB、TimescaleDB |
| 图 | 关系 | Neo4j |
| 文档 | 灵活模式 | MongoDB |
| 向量 | AI/ML | Pinecone、Milvus |
...
**DNS 解析**：
- 浏览器缓存 → 操作系统缓存 → DNS 解析器 → 根服务器 → TLD → 权威服务器
