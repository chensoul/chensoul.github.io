...
这篇文章涵盖了缓存、无服务器和分布式锁的核心概念：

**Uber CacheFront 3 个主要部分**：
| 部分 | 说明 |
|------|------|
| CacheFront 读写 | Redis + Docstore + MySQL 集成 |
| 多区域预热 | 跨区域 Redis 复制 |
| 分片 | 不同方案防止热分片 |
...
|------|------|
| 函数调用 | 同步/异步调用 |
| 分配服务 | Rust 编写，领导者 - 追随者 |
| Firecracker MicroVM | 轻量级 VM 管理器 |
| 组件存储 | 分块、加密、SnapStart |
