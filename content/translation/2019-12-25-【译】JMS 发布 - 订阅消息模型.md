...
date: 2019-12-25 12:00:00+08:00
draft: true
slug: jms-pub-sub-messaging-model
categories: ["translation"]
tags: ["jms"]
description: "在本文中，你将了解 JMS 发布 - 订阅 (publish-subscribe) 消息传递模型。"
canonicalURL: "https://jstobigdata.com/jms/jms-pub-sub-messaging-model/"
---

在本文中，你将了解 JMS 发布 - 订阅 (publish-subscribe) 消息传递模型。正如你在 [JMS 简介文章](https://jstobigdata.com/jms/jms-introduction-java-message-service/) 中所读到的，在发布/订阅模型中，客户端通过称为主题的中介将消息发送给多个接收者。发送者通常称为发布者，接收者称为订阅者。
...
> **注意**：所有发布/订阅系统通常都存在一定延迟，你编写的代码应该将延迟考虑在内。

## 普通消息订阅者的问题

普通消息订阅者（如上例中使用的订阅者）的问题在于，它不耐用。也就是说，如果消费者因某种原因宕机（关闭），则在恢复后它将无法接收先前的消息。
...
在实际场景中，你需要一种更可靠的方式来订阅主题。JMS 可以 `DurableConsumer` 解决这个问题。

## JMS 发布 - 订阅中的 DurableConsumer 示例

- 当必须接收某个主题的所有消息时，应使用持久订阅者（`DurableConsumer`）。
- JMS 确保在持久订阅者处于非活动状态时发布的消息由 JMS 保留，并在订阅者随后变为活动状态时进行传送。
- 当消费者在不活动时可以承受错过消息的后果时，将使用非持久订阅者。

下面的代码演示了 `DurableConsumer` 的用法。与前面的示例不同，此处的消费者从 `topic` 接收所有消息。链接至 [GitHub](https://github.com/jstobigdata/jms-parent-app/blob/master/jms-glassfish5/src/main/java/lab03/message/pubsub/DurableConsumerExample.java)。
...
在 JMS 2.0 中，多个订阅者可以监听一个主题，并且可以分配消息消费任务。有时，你可能需要对来自一个主题的多条消息进行负载平衡。这时你可以使用它来 `SharedConsumer` 分配消息消费负载。
...
这些都是 JMS 发布 - 订阅消息模型的一部分，你已经清楚地了解了 JMS 发布/订阅中**主题**的使用。你了解了不同的消费者，例如 `SharedConsumer`、`SharedDurableConsumer`、`DurableConsumer` 以及异步消息处理。请在下面的评论中分享你的想法。
