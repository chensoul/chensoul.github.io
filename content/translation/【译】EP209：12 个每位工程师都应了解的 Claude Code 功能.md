...
date: 2026-04-07 14:30:00+08:00
draft: true
...
- 本文是 ByteByteGo 通讯中的一期合集：主线是用 12 条能力清单帮助工程师快速建立对 **Claude Code** 功能面的心智模型；后半部分用短文补充 **Agentic RAG**、**REST** 与**负载均衡**等系统设计常识。
- 对 Claude Code 部分，作者隐含主张是：除了「能写代码」，现代编码代理的价值在于 **权限与工具治理**（Permissions、MCP、Plugins）、**可重复的工作方式**（`CLAUDE.md`、Skills、斜杠命令）、以及 **风险控制与成本**（Checkpoints、Compaction、Subagents）。

### 支撑要点

| 块 | 要点 |
| --- | --- |
| Claude Code 12 项 | 记忆与规范（`CLAUDE.md`）、执行治理（权限 / Plan / Hooks）、扩展（MCP、Plugins、Skills）、交互（`/context`、斜杠命令）、会话经济（Compaction）、并行（Subagents） |
| Agentic RAG | 相对「静态检索」增加代理决策与工具调用，流程：代理 → 记忆与策略 → 多工具取数 → 拼 prompt → LLM 回答 |
| REST | 以示意图收束原则、方法与约束（正文为导读，细节依赖配图） |
| 负载均衡 | 流量、SSL、会话、可用性、扩展、DDoS、健康检查七类典型价值 |

### 雷区与核对点

- **产品名与命令**：文中 `PreToolUse` / `PostToolUse`、`/context` 等需与工具链文档一致；若你本地版本重命名了能力，请以官方文档为准核对。
- **Agentic RAG**：这是高层流程说明，未涉及具体框架、向量库或编排实现，落地时需按栈补充工程细节。
- **配图依赖**：REST 段落在原文中明确依赖「下图」；若本地构建缺图，应补图或改写该段表述，避免「见下图」悬空。
