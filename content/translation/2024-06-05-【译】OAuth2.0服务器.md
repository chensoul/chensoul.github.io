...
![img](oauth-2-server/01.webp)在 GitHub 上注册新的 OAuth 应用程序
...
![img](oauth-2-server/02.webp)GitHub 应用程序已创建
...
就这样！现在您可以使用访问令牌向 GitHub 上的任何 API 端点发出 API 请求！您可以在<https://developer.github.com/v3/上查看> GitHub API 的完整文档。
...
![img](oauth-2-server/04.webp)在 Google API 控制台上为你的应用创建凭据
...
![img](oauth-2-server/06.webp)应用程序的服务器与 API 进行通信
...
![img](oauth-2-server/09.webp)授权请求示例
...
由于第三方脚本存在数据泄露的风险，因此为您的应用配置良好的内容安全策略非常重要，这样您就可以更加确信任意脚本无法在应用程序中运行。 OWASP 提供了有关配置内容安全策略的优秀文档，网址为<https://owasp.org/www-project-cheat-sheets/cheatsheets/Content_Security_Policy_Cheat_Sheet.html>
...
![在 Okta 中创建新应用程序](oauth-2-server/14.webp)在 Okta 中创建新应用程序
...
![GitHub 重新授权提示](oauth-2-server/15.webp)GitHub 在进行敏感更改时要求确认密码

该服务要求开发人员确认密码，然后才会透露秘密。当您尝试查看或更新敏感信息时，这种情况在亚马逊或 GitHub 网站上很常见。

![Dropbox“显示秘密”确认](oauth-2-server/16.webp)Dropbox 隐藏秘密，直到用户点击它
...
![GitHub 删除应用提示](oauth-2-server/17.webp)GitHub 要求确认删除应用程序
...
![GitHub 重置客户端密码提示](oauth-2-server/18.webp)GitHub 要求确认重置应用程序的机密
...
![img](oauth-2-server/19.webp)Twitter 授权屏幕的未登录视图
...
![img](oauth-2-server/20.webp)登录与未登录的用户流程

如果授权服务器需要通过 SAML 或其他内部系统对用户进行身份验证，则用户流程将如下所示

![img](oauth-2-server/21.webp)单独身份验证服务器的用户流程

在此流程中，用户登录后将被引导回授权服务器，在那里他们会看到授权请求，就像已经登录一样。

### 授权接口

![img](oauth-2-server/22.webp)OAuth 授权屏幕示例
...
“开放重定向”攻击是指授权服务器不需要重定向 URL 的精确匹配，而是允许攻击者构建重定向到攻击者网站的 URL。无论这最终是否被用来窃取授权代码或访问令牌，这也是一种危险，因为它可以用于发起其他不相关的攻击。有关开放重定向攻击的更多详细信息，请访问<https://oauth.net/advisories/2014-1-covert-redirect/。>
...
Google 的 API 是有效使用范围的绝佳示例。如需查看 Google OAuth API 支持的范围的完整列表，请访问其 OAuth 2.0 Playground，网址为<https://developers.google.com/oauthplayground/>
...
[RFC 9068](https://datatracker.ietf.org/doc/html/rfc9068)根据许多大型 OAuth 提供商的实际部署经验，定义了使用 JWT 作为访问令牌的标准方法。此规范定义了在包含有关身份验证、授权和身份的声明时使用的数据结构。有关更多详细信息，请参阅<https://oauth.net/2/jwt-access-tokens/。>
...
![img](oauth-2-server/32.webp)您已授权访问您的 Google 帐户的应用程序列表

该列表显示申请图标、名称以及申请获准范围的摘要。单击其中一个可展开该部分以显示更多详细信息。

![img](oauth-2-server/33.webp)有权访问您的 Google 帐户的某个应用程序的详细信息
...
![img](oauth-2-server/34.webp)您已授权访问 Twitter 帐户的应用程序列表
...
![img](oauth-2-server/35.webp)您已授权访问您的 GitHub 帐户的应用程序列表

GitHub 提供的列表包含应用程序上次使用情况的描述，以便让您了解如果应用程序在一段时间内未使用，是否可以安全地撤销其凭据。

单击某个应用程序可以提供有关该应用程序访问的更多详细信息。

![img](oauth-2-server/36.webp)有权访问你的 GitHub 帐户的一个应用程序的详细信息
...
![img](oauth-2-server/38.webp)设备发出 API 请求以获取设备代码

接下来，我们看到设备会显示代码以及 URL。

![img](oauth-2-server/39.webp)设备显示设备代码和URL

登录您的 Google 帐户后，访问该网址将显示一个界面，提示您输入设备上显示的代码。

![img](oauth-2-server/40.webp)Google 提示用户输入代码

输入代码并单击“下一步”后，您将看到标准 OAuth 授权提示，其中描述了应用程序请求的范围，如下所示。

![img](oauth-2-server/41.webp)Google 显示应用程序请求的范围

一旦您允许请求，Google 就会显示一条消息，提示您返回到您的设备，如下所示。

![img](oauth-2-server/42.webp)
...
[OpenID Connect 由OpenID 基金会](https://openid.net/)维护。 核心 OpenID Connect 规范以及许多扩展可在<https://openid.net/connect/上完整阅读。>
...
IndieAuth 是基于 OAuth 2.0 构建的去中心化身份协议，使用 URL 来识别用户和应用程序。它允许人们在登录和授权使用该身份的应用程序时使用他们控制的域作为身份。规范可在<https://www.w3.org/TR/indieauth/找到。>
...
![img](oauth-2-server/44.webp)

![img](oauth-2-server/45.webp)
...
![jwt.io](oauth-2-server/47.webp)jwt.io
