...
tags: [ "backend", "learning", "development" ]
...
这篇文章涵盖了后端学习、Git、虚拟化、容器化和 Netflix 计数器的核心概念：
...
| API | REST、GraphQL、gRPC、JWT、OAuth |
| 服务器托管 | AWS、Azure、Docker、K8s |
| DevOps | CI/CD、IaC、监控 |

**Git 工作流 5 步骤**：
| 步骤 | 命令 | 说明 |
|------|------|------|
| 工作目录→暂存 | git add | 未跟踪→已暂存 |
| 暂存→本地仓库 | git commit | 保存版本历史 |
| 本地→远程 | git push | 上传协作 |
| 远程→本地 | git pull/fetch | 更新本地 |
| 检查差异 | git diff | 显示更改 |
