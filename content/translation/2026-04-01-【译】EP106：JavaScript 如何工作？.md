...
**Kafka 消息丢失场景**：
- **Producer**：需配置适当的 acks 和 retries
- **Broker**：异步刷盘可能导致丢失，副本配置很重要
- **Consumer**：自动提交可能过早确认，建议结合同步/异步提交

**微服务 9 个最佳实践**：
1. 为失败设计（断路器、舱壁）
2. 构建小服务（单一职责）
3. 轻量级协议（REST、gRPC、消息代理）
4. 服务发现（Consul、Eureka、K8s）
5. 数据所有权（减少耦合）
6. 弹性模式（重试、缓存、限流）
7. 所有层面安全
8. 集中式日志
