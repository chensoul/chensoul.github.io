---
title: "关于"
---

## 关于我

> 健康，爱情和使命，按照这个顺序，其它的都不重要。
> ------纳瓦尔

我是 ChenSoul，一个来自中国武汉的开发者，喜欢[编程](/categories/techlog)、[跑步](https://run.chensoul.cc/)。不定期更新[博客](https://blog.chensoul.cc)，分享学习心得。

## 更新

- 2021-09-09 » 注册 chensoul.com 域名，并使用 Hugo 主题构建博客
- 202?-0?-0? » 将博客域名改为 blog.chensoul.cc 
- 2026-02-24 » 从 Hugo 迁移到 Astro，使用 [astro-lhasa](https://github.com/achuanya/astro-lhasa) 主题。
- 2026-02-25 » 参考 [Yiran's Blog](https://zdyxry.github.io/running/)，增加[跑步](/running)页面。
- 2026-02-27 » 使用 [https://zdyxry.github.io/](https://zdyxry.github.io/) 的主题配色

## 知识共享协议

本博客前端源码基于 <a href="https://github.com/chensoul/astro-lhasa" target="_blank" rel="noopener noreferrer">astro-lhasa</a> 项目构建，遵循 MIT License 发布：

> Copyright (c) 2026 ChenSoul 本软件可自由使用、复制、修改、分发，前提是保留原始版权声明。软件按"原样"提供，不附带任何明示或暗示的担保。
> 
> <a href="https://creativecommons.org/licenses/by-nc-nd/4.0/deed.zh-hans" target="_blank" rel="noopener noreferrer">CC BY-NC-ND 4.0 国际许可协议</a>，转载请务必注明出处，禁止商用及修改。
