...
echo "export HOMEBREW_CDN_DOMAIN="https://mirrors.ustc.edu.cn/homebrew-bottles"" >> ~/.zshrc
```

> **提示**：镜像源配置已包含在 dotfiles 的 `.zshrc` 中，运行 `install.sh` 会自动应用。
...
| `~/.zshrc`                 | Zsh Shell 配置（含别名和函数） | `private_dot_zshrc`                    |
...
├── private_dot_zshrc             # Zsh Shell 配置（含别名和函数）
├── dot_config/
