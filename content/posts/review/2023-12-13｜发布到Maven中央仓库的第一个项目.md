...
配置一个 maven-release.yml 文件，文件内容参考如下：
...
jobs:
  publish:
    ...
    steps:
      ...
      - name: Maven Release
        run: mvn -ntp -B -U clean source:jar javadoc:jar deploy site scm-publish:publish-scm -P release -DautoReleaseAfterClose=true
        env:
