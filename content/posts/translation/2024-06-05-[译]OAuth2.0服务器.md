...
![img](github_create_new_application.webp)在 GitHub 上注册新的 OAuth 应用程序
...
![img](github_application_created.webp)GitHub 应用程序已创建
...
![img](google-create-credentials.webp)在 Google API 控制台上为你的应用创建凭据
...
![img](user_browser_client_server.webp)应用程序的服务器与 API 进行通信
...
![img](okta_oauth-diagrams-1.webp)
...
![img](okta_oauth-diagrams_20170622-01-1.webp)授权请求示例
...
![img](sfsafariviewcontroller-example.webp)
...
![在 Github 上创建新的应用程序](github_create_new_application-02.webp)在 GitHub 上创建新应用程序

最好向开发人员说明您从他们那里收集的信息是否会显示给最终用户，或者是否仅供内部使用。

Foursquare 的应用程序注册页面会要求提供类似的信息，但他们还会要求提供简短的标语和隐私政策 URL。这些信息会在授权提示中显示给用户。

![在 Foursquare 上创建新应用程序](foursquare_create_new_application.webp)在 Foursquare 上创建新应用程序

由于使用旧式隐式授予类型的安全考虑，某些服务（例如 Instagram）默认为新应用程序禁用此授予类型，并要求开发人员在应用程序的设置中明确启用它，如下所示。

![在 Instagram 上创建新应用程序](instagram_create_new_application.webp)在 Instagram 上创建新应用程序
...
![在 Okta 中创建新应用程序](okta_new_application.webp)在 Okta 中创建新应用程序
...
![GitHub 重新授权提示](github_confirm_password.webp)GitHub 在进行敏感更改时要求确认密码

该服务要求开发人员确认密码，然后才会透露秘密。当您尝试查看或更新敏感信息时，这种情况在亚马逊或 GitHub 网站上很常见。

![Dropbox“显示秘密”确认](dropbox_show_secret.webp)Dropbox 隐藏秘密，直到用户点击它
...
![GitHub 删除应用提示](github_delete_application.webp)GitHub 要求确认删除应用程序
...
![GitHub 重置客户端密码提示](github_reset_client_secret.webp)GitHub 要求确认重置应用程序的机密
...
![img](twitter_logged_out_auth_screen.webp)Twitter 授权屏幕的未登录视图
...
![img](okta_oauth-diagrams_20170622-04.webp)登录与未登录的用户流程

如果授权服务器需要通过 SAML 或其他内部系统对用户进行身份验证，则用户流程将如下所示

![img](okta_oauth-diagrams_20170622-05.webp)单独身份验证服务器的用户流程

在此流程中，用户登录后将被引导回授权服务器，在那里他们会看到授权请求，就像已经登录一样。

### 授权接口

![img](oauth_authorization_screen.webp)OAuth 授权屏幕示例
...
![img](benwerd-twitter-checkboxes.webp)
...
![img](google-authorization-list.webp)您已授权访问您的 Google 帐户的应用程序列表

该列表显示申请图标、名称以及申请获准范围的摘要。单击其中一个可展开该部分以显示更多详细信息。

![img](google-authorization-details.webp)有权访问您的 Google 帐户的某个应用程序的详细信息
...
![img](twitter-authorization-list-1024x719.webp)您已授权访问 Twitter 帐户的应用程序列表
...
![img](github-authorization-list-1024x462.webp)您已授权访问您的 GitHub 帐户的应用程序列表

GitHub 提供的列表包含应用程序上次使用情况的描述，以便让您了解如果应用程序在一段时间内未使用，是否可以安全地撤销其凭据。

单击某个应用程序可以提供有关该应用程序访问的更多详细信息。

![img](github-authorization-details.webp)有权访问你的 GitHub 帐户的一个应用程序的详细信息
...
![img](getting-device-code-1024x275.webp)设备发出 API 请求以获取设备代码

接下来，我们看到设备会显示代码以及 URL。

![img](device-code-display-1024x275.webp)设备显示设备代码和URL

登录您的 Google 帐户后，访问该网址将显示一个界面，提示您输入设备上显示的代码。

![img](google-1-enter-the-code.webp)Google 提示用户输入代码

输入代码并单击“下一步”后，您将看到标准 OAuth 授权提示，其中描述了应用程序请求的范围，如下所示。

![img](google-2-scope.webp)Google 显示应用程序请求的范围

一旦您允许请求，Google 就会显示一条消息，提示您返回到您的设备，如下所示。

![img](google-3-return-to-device.webp)
...
![img](indieauth-login-prompt-1024x436.webp)
...
![img](indieauth-authorization-prompt-aaronpk-1024x969.webp)

![img](indieauth-authorization-prompt-wordpress.webp)
...
![img](oauth-playground-1024x752.webp)
...
![jwt.io](jwt-io-1024x792.webp)jwt.io
