...
![Application Architecture: 401 Unauthorized](app-arch-401.webp)
...
![Application Architecture: JSON Web Token (JWT)](app-arch-jwt.webp)

然后，客户端将存储 JWT，并且每个后续请求将通过 Authorization 标头传递它。当服务器应用程序收到带有 JWT 的请求时，它将验证它是否是有效令牌，如果是，则允许请求继续。

![Application Architecture: Request with JSON Web Token (JWT)](app-arch-with-jwt-200.webp)
...
![Spring Initializer](start-spring-io.webp)
...
![Spring Security Login](please-sign-in.webp)
...
![Application Architecture: JSON Web Token (JWT)](app-arch-jwt-02.webp)
...
![Httpie with Authorization](httpie-auth.webp)
...
![Httpie Response Success](httpie-success.webp)
