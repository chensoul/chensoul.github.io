...
date: 2026-03-31 16:27:00+08:00
draft: true
slug: "algorithms-you-should-know-before-system-design-interview"
...
这篇文章列出了系统设计面试中常见的重要算法，按重要性分级：

**五星重要**（必须理解）：
- **一致性哈希**：分布式缓存、数据库分片
- **布隆过滤器**：判断元素是否存在（缓存穿透、黑名单）
- **Merkle 树**：数据同步、区块链、Git
...
**一星重要**（高级候选人）：
- **Raft/Paxos**：分布式共识
- **操作转换**：协同编辑（如 Google Docs）
- **分层时间轮**：定时器调度

面试建议：
- 重点理解**应用场景**而非实现细节
- 能说出"什么场景下用什么算法"比能写代码更重要
- 对于高级岗位，了解分布式共识算法是加分项

这些算法在实际系统中无处不在，理解它们对设计高效系统很有帮助。
