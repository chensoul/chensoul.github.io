...
date: 2026-03-31 16:10:00+08:00
draft: true
...
这篇文章是 Kubernetes 系列的开篇，主要把基础概念讲清楚了：

- **Kubernetes 的本质**：容器编排平台，自动化部署、扩展和管理容器化应用
- **历史渊源**：来自 Google 内部的 Borg 系统，2014 年开源
- **核心架构**：控制平面（大脑）+ 工作节点（执行者）
- **最小单元**：Pod，是最小可部署单位，可以包含一个或多个容器
- **为什么叫 k8s**：k 和 s 之间有 8 个字母，这种缩写在技术圈很常见（类似 i18n）

对于要不要上 K8s 这个问题，这篇文章的价值在于它没有一味鼓吹，而是会讨论权衡（tradeoffs）。实际生产中，很多团队其实不需要 K8s 的完整能力，但理解 K8s 的设计思想对做架构决策很有帮助。
