---
title: "【译】Redis 速成课程"
date: 2026-03-31 16:12:00+08:00
draft: true
...
这篇文章把 Redis 为什么快讲得比较透彻：

- **内存存储**：比磁盘快 1000-10000 倍，这是基础
- **哈希表 O(1)**：全局哈希表实现快速查找
- **丰富的数据结构**：String、List、Set、Hash、SortedSet 等，每种都有优化的底层实现
- **SDS 设计**：预分配空间、记录长度，避免溢出和重复计算
- **持久化选项**：AOF（日志）、RDB（快照）、两者结合、或无持久化

实际使用中：
- 做缓存时，Redis 几乎是标配
- 做消息队列时，List 和 Stream 是常用选择
- 做计数器/去重时，HyperLogLog 和 Bitmap 很实用
- 做地理位置时，GeoHash 内置支持

理解 Redis 的底层实现，有助于在设计和调优时做出更合理的决策。
