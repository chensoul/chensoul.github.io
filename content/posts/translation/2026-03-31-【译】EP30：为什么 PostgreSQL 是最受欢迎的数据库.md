...
date: 2026-03-31 17:38:00+08:00
draft: true
...
**PostgreSQL 为什么受欢迎**：
- 一个数据库支持多种用例（OLTP + OLAP）
- 丰富的扩展生态（PostGIS、Timescale、Citus 等）
- HTAP 架构，混合事务和分析处理

**代理对比**：
- 正向代理：客户端→代理→互联网（代表客户端）
- 反向代理：互联网→代理→服务器（代表服务器）

**时间表示**：
- UTC、GMT：标准时间
- TAI：原子时，无闰秒
- Unix 时间戳：从 1970-01-01 开始的秒数
- 闰秒：偶尔出现，需要特殊处理

理解这些基础知识对系统设计和技术选型至关重要。
