...
- 基于代理的架构（避免客户端复杂性）
- 一致性哈希分区（1024 个固定分片）
- 法定协议复制（5 区域，读/写法定人数各 3）
- 自动故障转移（无需领导者重新选举）
- 跨数据中心异步复制

**为什么不用 Redis**：
- Redis 是单线程、内存绑定
- JunoDB 需要多核支持、CPU 绑定（加密等）

**使用案例**：
1. 缓存（短/长 TTL 令牌、用户偏好）
2. 幂等性（防止重复付款）
3. 速率限制（计数器跟踪）
4. 跨数据中心复制（弥补 Oracle 慢复制）

JunoDB 展示了如何为特定需求（高并发、加密、多核）设计和构建定制数据库系统。
