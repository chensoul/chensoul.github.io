...
- 这篇文章主要在展示 Anthropic 如何观察 Claude 的内部计算过程，而不只是罗列几个有趣实验。后面关于多语言、写诗、数学、幻觉和越狱的案例，都是围绕这条主线展开的。
- “Claude 如何做数学”与“当 Claude 的推理受到动机驱动时”两节可以连在一起看。原文想说明的是：模型写出来的解释或推理链，有时并不等于内部真实发生的计算过程。
- “Claude 在概念中思考”这一节的重点，不只是 Claude 会多语言，而是不同语言在更深层可能共享同一套概念表示。这里的重点在于概念层的共享，而不只是翻译能力。
- “为什么会出现幻觉”这一节有一个重要前提：Claude 的默认状态更接近拒答，而不是默认胡编。原文把幻觉解释为识别系统对“已知实体”的误判，从而覆盖了拒答机制。
- 原文含推广段落，已略。结论部分也明确提醒了边界：成功案例有限、观察对象是替代模型、分析成本高，因此这些结果更适合理解为阶段性观察，而不是对 Claude 内部机制的最终定论。
