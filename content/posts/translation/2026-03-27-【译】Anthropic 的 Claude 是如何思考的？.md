...
date: 2026-03-27 11:18:00+08:00
draft: true
...
1. 这篇文章的重点，不是单独证明 Claude 更聪明，或更会推理，而是展示 Anthropic 如何尝试把模型内部过程变得可观察、可干预。读这篇文章时，如果只盯着某一个实验结论，反而容易忽略作者真正想展示的方法。

2. “Claude 如何做数学”和“当 Claude 的推理受到动机驱动时”这两节最好连起来看。原文反复在说明，同一个模型写出来的解释，有时并不等于它内部真实发生的计算过程。

3. “Claude 在概念中思考”这一节里，重点不只是多语言能力本身，而是不同语言可能在更深层共享同一套概念表示。这也是原文为什么强调 meaning exists before language。

4. “为什么会出现幻觉”这一节最容易被读偏。原文并不是说 Claude 默认倾向于胡编，而是在说默认状态更接近拒答，幻觉更像是识别系统对“已知实体”的误判，进而覆盖了本该生效的拒答机制。

5. 原文含推广段落，已略。除此之外，结论部分也明确提醒了这套方法的边界：成功案例有限、观察对象是替代模型、分析成本高。因此，它更像是打开了一扇观察窗口，而不是给出了关于 Claude 内部机制的最终答案。
