...
提及处理程序方法的顺序并不重要。 Spring将首先寻找最具体的异常处理方法。
...
>
> 当您尝试调用系统中不存在的API时，会出现此异常。尽管我们通过 `ResponseEntityExceptionHandler` 类实现其处理程序，但异常仍被重定向到 `DefaultHandlerExceptionResolver` 。
>
