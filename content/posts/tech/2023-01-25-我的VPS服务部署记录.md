...
BACKUP_POSTFIX=$(date +%Y%m%d%H)   # 如 20260303_16
...
# 删除超过1天的备份文件
ls -t --time=birth "$BACKUP_DIR"/*.sql | awk -v now=$(date +%s) '{cmd="stat -c %W "$0; cmd | getline b; close(cmd); if (now - b > 86400) print $0}' | xargs rm -f
...
docker cp n8n:/home/node/backup_workflows/. /tmp/workflows
for f in /tmp/workflows/*; do
  [ -f "$f" ] || continue
  name=$(jq -r '.name' "$f" 2>/dev/null)
  [ -n "$name" ] && [ "$name" != "null" ] && mv -u "$f" "$BACKUP_DIR/workflows/${name}.json"
done
