...
除了在安装时可以进行配置之外，还可以通过 Dashboard
网页进行配置，或者直接修改配置文件进行配置。可以参考阿里云的文档[在 OpenClaw 中配置 Coding Plan](https://help.aliyun.com/zh/model-studio/openclaw-coding-plan#0c6a73ae73mqr)。
...
如果安装 OpenClaw 的服务器 **没有图形界面**：`openclaw dashboard` 不会自动弹浏览器， 终端会打出一段说明（**程序输出**
，不是要你执行的命令）。
...
端口在配置里 `gateway.port`，默认 18789。 若只在本机用，可把 `gateway.host` 设为 `127.0.0.1`； 要远程访问再绑 `0.0.0.0`，并建议开
`gateway.authToken`，详见 [Gateway 文档](https://docs.openclaw.ai/gateway)。
...
把 bot 拉进群。如果配了 `requireMention: true`，得在群里 @ 它才会回。 想看到所有群消息：在群里设成管理员，或 @BotFather 里对
bot 用 `/setprivacy` 关隐私。
...
- **群组**：`telegram:-100xxxxxxxxxx`（群组 chat_id，一般为负的 long 数字）。 chat_id 可在群组里先发一条消息，用
  `openclaw pairing list telegram` 等看到；或通过 Telegram Bot API 获取。

### 小龙虾初始化

刚开始的时候，和小龙虾聊天，需要告诉它你是谁，以下是一个模版。
...
技能从三个位置加载，优先级从高到低：**工作区** `~/.openclaw/workspace/skills`、**本地管理** `~/.openclaw/skills`、**内置**
（随安装）。同名时工作区覆盖本地，本地覆盖内置。
...
- **手动**：把技能目录放到 `~/.openclaw/workspace/skills/`
  下，结构符合 [Creating Skills](https://docs.openclaw.ai/tools/creating-skills) 即可。

**配置**

在 `~/.openclaw/openclaw.json` 的 `skills` 段里可做：启用/禁用某个技能、配 API Key、设 `env`、限制内置技能（`allowBundled`
）、调整加载目录（`load.extraDirs`）等。 改完一般无需重启 Gateway，下次 agent 执行时会生效；若未生效可
`openclaw gateway restart`。
...
任务持久化在 `~/.openclaw/cron/jobs.json`，重启 Gateway 不会丢。更多选项（如
webhook、隔离/主会话模式）见 [Cron 文档](https://docs.clawd.bot/cron)。
...
1、使用 clinkding 技能保存书签到 Linkding

```bash
clinkding config -u https://linkding.chensoul.cc -t YOUR_TOKEN
...
clinkding
技能安装之后，给机器人发消息，比如：“保存书签 https://github.com/AlexAnys/awesome-openclaw-usecases-zh”，机器人就会调用配置的
clinkding 服务的 API 接口保存书签。
...
**用 Dashboard 改配置**：
`openclaw dashboard` 打开网页，左侧 **配置** → **All Settings** → **RAW**， 编辑的就是 `openclaw.json`。改完点 **Save** 再
**Update**。

> 注意：已有渠道、模型时别整段覆盖 Raw JSON，只改要改的那几块 （如 `models`、`agents`、`gateway`）。缺键再补。

接阿里云百炼 **Coding Plan**：在 Raw JSON 里加 `models.providers` 和  `agents.defaults.model`，把 `YOUR_API_KEY` 换成真实
Key。
...
说明当前用的 agent（例如 `main`）没配这个模型的认证。 要么给这个 agent 配一遍：`openclaw agents add main`，按提示选
provider、填 Key； 要么从已经配好的 agent 拷认证文件过去，例如：
...
认证是按 agent 存的，路径在 `~/.openclaw/agents/<agent>/agent/auth-profiles.json`。 Onboarding 一般只给默认 agent 写了认证，换了
agent 或迁过目录就要单独配或拷一份。

**报错：clawhub install xxx — Rate limit exceeded**  
ClawHub registry 对请求做了限流（同一 IP 或短时间请求过多）。处理方式：先试 `npx clawhub install <技能名>`，往往能正常安装；若仍失败，等
10–30 分钟再试、关 VPN 或换网络、避免连续重试（会拉长限流时间）。
