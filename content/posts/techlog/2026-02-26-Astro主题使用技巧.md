...
注意图片路径前面没有 `/`，图片地址是相对路径。Photosuite 会将图片地址替换为 {imageBase}/images/example-picture.jpg，imageBase 在
astro.config.ts 中由 photosuite 配置。

> 如果使用绝对路径，比如 `![example-picture](example-picture.jpg)`, Photosuite 不会进行替换。
...
```bash
rclone tree r2:cos
# 复制到桶里的子目录，例如 cos 桶下的 images。不删远程多余文件（默认就是这样）
# -P 显示进度
rclone copy public/images r2:cos/images -P
