...
categories: [ "techlog" ]
tags: ['spring-ai']
cover: /thumbs/tools.svg
description: "本文介绍如何在 Spring AI 中使用工具调用（Tool Calling / Function Calling）：让大模型在对话中按需调用你定义的 Java 方法（查员工、查请假、获取当前日期等），并根据工具返回结果生成回复。本文基于 chat-model 模块，通过 @Tool 注解和 ChatClient.defaultTools() 实现员工与日期相关的工具调 ..."
---
