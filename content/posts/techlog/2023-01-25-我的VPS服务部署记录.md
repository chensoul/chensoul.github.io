...
配置 AlmaLinux 9 源（可选，以阿里云镜像为例）：

```bash
# 备份并替换为阿里云镜像（保留默认源时可跳过）
...
AlmaLinux 9 可使用 Nginx 官方 RHEL 9 源安装：

```bash
# 添加 Nginx 官方源（适用于 RHEL 9 / AlmaLinux 9）
...
docker cp n8n://home/node/credentials.json ${BACKUP_DIR}/n8n-credentials.json
...
# 时钟同步（chronyd 已常驻，可选：定期强制同步）
# */20 * * * * chronyc -a makestep > /dev/null 2>&1
