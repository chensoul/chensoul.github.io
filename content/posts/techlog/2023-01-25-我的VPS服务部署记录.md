...
categories: [ "techlog" ]
tags: ['vps','almalinux']
description: "本文记录了在 VPS 服务器上部署各种服务的详细步骤，包括服务器初始化、Docker 环境配置、以及多个自托管服务的部署过程。 ..."
---
...
BACKUP_POSTFIX=$(date +%Y%m%d_%H)   # 如 20260303_16
...
按备份时间戳恢复（备份目录 `postgres/`，文件命名：`<库名>_<YYYYMMDD_HH>.sql`）。将 `BACKUP_POSTFIX` 改为要恢复的那次备份时间（如 `20260303_16`），在备份仓库根或指定目录下执行。
...
BACKUP_POSTFIX="20260303_16"   # 改为实际备份时间
...
DATE=$(date +%Y%m%d)   # 如 20260303
WORKFLOWS_DIR="$BACKUP_DIR/$DATE/workflows"
...
docker cp n8n:/home/node/credentials.json "$BACKUP_DIR/$DATE/credentials.json"
...
git pull
git add .
git commit -m "backup n8n for $DATE"
git push origin main
```

备份目录为 `n8n/`，按日期分子目录：`n8n/YYYYMMDD/workflows/*.json`、`n8n/YYYYMMDD/credentials.json`。恢复时把对应日期目录拷入 n8n 容器再用 CLI 导入。

**恢复工作流**（`DATE` 改为要恢复的备份日期，如 `20260303`）：

```bash
BACKUP_DIR="/opt/vps-backup/n8n"
DATE="20260303"

docker cp "$BACKUP_DIR/$DATE/workflows" n8n:/home/node/restore_workflows
