...
### 问题 1：插件加载失败（CRLF 换行符）

**症状**：启动时报错 `command not found: ^M`

**原因**：Git clone 时文件带有 Windows 换行符（CRLF），zsh 无法正确执行。

**解决**：

```bash
# 批量修复所有 zsh 文件的换行符
find ~/.zinit/plugins/ -name "*.zsh" -type f -exec sed -i '' 's/\r$//' {} \;
```
...
**解决**：确保 `sdkman-init.sh` 存在且正确 source：
...
| zinit    | 插件管理器    | `git clone https://github.com/zdharma-continuum/zinit.git ~/.zinit/bin` |
