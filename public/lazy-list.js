      ...
      if (artalk && artalk.enabled) {
        ...
        next.forEach(function (el) {
          if (!el.querySelector(".artalk-comment-count")) {
            ...
            if (!anchor) return;
            var wrap = artalk.wrapperSelector
              ? el.querySelector(artalk.wrapperSelector)
              : null;
            var span = document.createElement("span");
            span.className =
              "artalk-comment-count " +
              batchCls +
              " absolute right-0 top-1/2 -translate-y-1/2 inline-flex items-center justify-center rounded-full text-xs font-medium shadow-sm";
            span.setAttribute(
              "style",
              "width:30px;height:16px;background-color:var(--comment-badge-bg);color:var(--color-background)"
            );
            span.setAttribute(
              "data-page-key",
        ...
        var cfg =
          (typeof window !== "undefined" && window.__artalkConfig) || {};
        var server = artalk.server || cfg.server;
        var site = artalk.site || cfg.site;
        if (
          typeof window !== "undefined" &&
          typeof window.Artalk !== "undefined" &&
          server &&
          site
        ) {
          try {
        ...
        next.forEach(function (el) {
          var badge = el.querySelector("." + batchCls);
          if (badge) badge.classList.remove(batchCls);
        });
      }
      shown += next.length;
