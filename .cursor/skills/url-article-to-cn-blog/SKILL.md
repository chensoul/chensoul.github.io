...
对于**长篇文章**（例如超过 5000 词或章节较多），可按章节分段翻译：

1. **拆分策略**：以原文的**一级或二级章节**为单位拆分（例如 `## Section 1`、`## Section 2`…）
2. **翻译顺序**：按章节顺序依次翻译，确保术语前后一致
3. **合并输出**：所有章节翻译完成后，合并为**单个 Markdown 文件**，保持原文章节结构
4. **配图处理**：全文统一编号（01.webp、02.webp…），不按章节分段编号
5. **一致性检查**：合并后通读全文，确保：
   - 术语翻译前后统一
   - 章节之间过渡自然
   - 无重复或遗漏的内容

**注意**：分段翻译仅为执行策略，最终交付物仍是**一个完整的 `.md` 文件**，而非多个分节文件。
...
📁 文件路径：content/translation/{YYYY-MM-DD}-【译】{中文标题}.md
🔗 原文链接：{canonicalURL}
🏷️ Slug:    {slug}
📅 日期：    {date}
📝 Draft:   {draft}
🖼️ 配图：    {N} 张 → public/images/{slug}/01.webp ~ {NN}.webp
🔍 审核结果:
  ✓ slug 与 URL 一致
  ✓ 文章标题符合规范
  ✓ 代码与原文一致
