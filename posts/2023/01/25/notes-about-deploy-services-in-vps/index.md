...
```bash
# 下载最新版本的 Docker Compose（请访问 GitHub 获取最新版本号）
...
RSSHub 是一个开源、简单易用、易于扩展的 RSS 生成器，可以为任何网站生成 RSS 订阅源。
...
2. 配置 nginx 配置文件 `/etc/nginx/conf.d/rsshub.conf`：
...
Hoarder 是一款可自托管的书签应用程序，带有 AI 智能功能，适合数据收集爱好者使用。
...
Blinko 是一个现代化的链接管理工具，可以帮助你组织和分享链接。
...
remark42 是一个自托管的评论系统，支持多种认证方式（GitHub、Google、Telegram 等），可以替代 Disqus。

项目地址：https://github.com/umputun/remark42
...
docker logs -f remark42
```

> **注意**：需要先在 GitHub 和 Google 创建 OAuth 应用，获取 Client ID 和 Client Secret，然后配置到环境变量中。

```nginx
server {
    listen 80;
    listen [::]:80;
    server_name comment.chensoul.cc;
...
# 手动更新证书
~/.acme.sh/acme.sh --renew -d chensoul.cc --force
...
# 特定服务的日志
tail -f /var/log/nginx/<service_name>.log
```
