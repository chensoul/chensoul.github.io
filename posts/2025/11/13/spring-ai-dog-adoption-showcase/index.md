...
在 `SchedulerApplication` 中，我们定义了一个本地函数，并用 `@Tool` 标注：
...
在 `AdoptionsApplication` 中，我们没有直接写工具代码，而是通过 **MCP (Model Context Protocol)** 连接到了远程的 Scheduler 服务。

MCP 是一个开放标准，允许 AI 模型连接到任何数据源或工具。

```java
// 客户端连接到 http://localhost:8081
var mcp = McpClient.sync(HttpClientSseClientTransport.builder("http://localhost:8081").build()).build();

// 注册 MCP 工具
ai.defaultToolCallbacks(new SyncMcpToolCallbackProvider(mcpSyncClient))
```
